...
// Sample Pali data - you can replace this with your own file
const paliData = [
  {
    id: 1,
...
// Majjhima Nikaya suttas data (1-152)
const generateSuttaData = (number) => {
  ...
  // Video ID and index mapping for known sutras
  const audioMapping = {
  ...
  const suttaName = suttaNames[number] || `Sutta ${number}`;
  const audioData = audioMapping[number];
